...
def filterTooShort(rawData, minTokens):
    return rawData[findRightSize(rawData, minTokens)]

def findRightSize(rawData, minTokens):
    results = [False] * len(rawData["text"])
    ...
    for i in range(len(rawData["text"])):
        if workDone % fiveP == 0:
            print(f"{int((workDone/fiveP) * 5)}%")

        cleaned = textToCleanedWords(rawData["text"][i])

        if (len(cleaned) >= minTokens):
            results[i] = True
...
def countTokens(data):
    ...
    for i in range(len(data)):
        ...
        if (not isinstance(text, str)):
            ...
            words = textToCleanedWords(text)
            results.append(len(words))

    return results

def getRelRatio(tokenCount, data):
    ...
    currentProgress = 0
    stepSize = int((max(tokenCount) + 2) / 100)

    for value in range(-1, max(tokenCount)):

        if (value + 1) % stepSize == 0:
            print(f"{(value + 1) / stepSize}% done")
        currentProgress += 1

        unreliable = 0
        total = 0

        for i in range(len(tokenCount)):

            if (tokenCount[i] == value):
                unreliable += data["label"][i]
                total += 1

        if (not total == 0):
            counts.append(unreliable)
            totals.append(total)
            labels.append(value)


    # Now to turn them into ratios
    ratios = []
    ...
    for i in range(0, len(counts)):
        ...
        ratios.append(unrelTmp / totalTmp)
...
def crossValidateLog(df, folds):
    ...
    for i in range(0, folds):
        ...
        print("Training log. regression")
        logreg = LogisticRegression()
        logreg.fit(trainingSet.drop("label", axis = 1), trainingSet["label"])

        print("Getting accuracy")
        predicted = logreg.predict(validationSet.drop("label", axis = 1))
        cm = confusion_matrix(validationSet["label"], predicted)

        correct = cm[0][0] + cm[1][1]
        acc = correct / (correct + cm[0][1] + cm[1][0])
